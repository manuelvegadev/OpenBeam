...
> **Source-of-truth rule.** This document is the canonical specification. Both the macOS implementation in CamNDI and the Linux companion app implement *against this file*. Whenever a decision affects what goes on the wire — packet shapes, crypto primitives, key derivation strings, limits, port numbers — update this file first, then update the implementations to match.
...
The reference Mac implementation generates test vectors on first run and writes them to `~/Library/Caches/CamNDI/clipsync/test-vectors.json`. Format:
